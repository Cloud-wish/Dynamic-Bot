...
@commands.cmd((command_dict["remove"]["weibo"], "weibo", "weibo"), (command_dict["remove"]["bili_dyn"], "bili_dyn", "dynamic"), (command_dict["remove"]["bili_live"], "bili_live", ("status", "title", "cover")))
async def remove_push(cmd: str, typ: str, subtypes: str|tuple[str], msg: Message, uid: str):
    bot_id = msg["bot_id"]
    user = msg["sender"]
    ...
    for _ in [1]:
        for subtype in subtypes:
            db.remove_push(typ, subtype, uid, bot_id, guild_id, channel_id)
        if not db.exist_push_user(uid, typ):
            resp = await remove_crawler(uid, typ)
            if not resp["success"]:
                for subtype in subtypes: # revert changes
                    db.add_push(typ, subtype, uid, bot_id, guild_id, channel_id)
                if resp and resp["result"]["code"] == 11:
                    reply_content = f"UID：{uid} 未开通直播间"
                else:
                    reply_content = f"UID：{uid} 的{type_dict[typ]}推送删除失败！"
                break
        reply_content = f"UID：{uid} 的{type_dict[typ]}推送删除成功！"
