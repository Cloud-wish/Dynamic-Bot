...
dyn_handlers = HandlerTableDef()

@handlers.handler("bili_dyn")
async def dispatch_dyn_push(typ: str, msg: dict):
    msg = msg_preprocess(msg, msg["type"])
    try:
        await dyn_handlers(msg["subtype"], msg)
...
@dyn_handlers.handler("dynamic")
async def dyn_handler(subtype: str, data: dict):
    await common_push_handler(subtype, data)

@dyn_handlers.handler("comment")
async def dyn_cmt_handler(subtype: str, data: dict):
    pass