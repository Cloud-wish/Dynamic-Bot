...
async def weibo_pic_builder(subtype: str, uid: str, data: dict) -> list[str]:
    ...
    content.append('[CQ:image,file=file:///'+pic_path+']')
    content.append(f"微博地址：{'https://m.weibo.cn/detail/' + data['id']}")
    return content
    ...
    if("reply" in data):
        ...
        for i, pic_info in enumerate(data['reply']['pics'], 1):
            if pic_config_dict["weibo_pic_save"]:
                ...
                content.append('[CQ:image,file='+pic_info+']')
        content.append('\n')
    content.append(f"原微博地址：{'https://m.weibo.cn/detail/' + data['root']['id']}")
    return content
...
async def dynamic_builder(subtype: str, uid: str, data: dict) -> list[str]:
    ...
    if data["dyn_type"] == 2 or data["dyn_type"] == 4: # 动态
        ...
        content.append("[无法解析该动态，请点击地址查看]")
    if not data.get("is_retweet", False):
        content.append("\n")
        if data["dyn_type"] == 8:
            content.append("视频地址：\n")
...
async def dynamic_comment_builder(subtype: str, uid: str, data: dict):
    ...
    if("reply" in data):
        content.append(f"原评论：\n{data['reply']['text']}\n")
        content.append('\n')
    content.append(f"原动态地址：{'https://t.bilibili.com/' + data['root']['id']}")
    return content
...
async def status_builder(subtype: str, uid: str, data: dict) -> list[str]:
    ...
    if(data['now'] == "1"):
        ...
        content.append(f"地址：https://live.bilibili.com/{data['user']['room_id']}")
    elif(data['pre'] == "1"):
